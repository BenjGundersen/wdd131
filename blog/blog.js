...
articles.forEach(article => {
    ...
    leftSideDiv.innerHTML = `
		<p><strong>Date:</strong> <span class="articledate">${article.date}</span></p>
		<p><strong>Ages:</strong> ${article.ages}</p>
		<p><strong>Genre:</strong> ${article.genre}</p>
		<p><strong>Rating:</strong> ${article.stars}</p>
	`;

    const middleSideDiv = document.createElement('div');
    middleSideDiv.classList.add('middleside');
    middleSideDiv.innerHTML = `
		<h2>${article.title}</h2>
		<img src="${article.imgSrc}" alt="${article.imgAlt}" />
		<p>${article.description}</p>
	`;

    articleRow.appendChild(leftSideDiv);
    articleRow.appendChild(middleSideDiv);
